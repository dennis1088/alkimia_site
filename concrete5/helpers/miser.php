...
class MiserHelper {
		...
		private $endis_html_min			= 1;			// Enable/disable HTML minification			0=OFF, 1=MINIFY
		// Combining
		...
		private $eu_cookie				= NULL;			// The cookie name to detect when requiring GA to be removed from the page.
		...
		function __construct($root = NULL) {
			...
			$this->ignore_keys 		= array( 'google_ad', 'show_ads.js', 'gmodules.com');
		...
		Protected function do_optimise( $html ){			
			...
			if (!empty($this->eu_cookie) && empty($_COOKIE[$this->eu_cookie])) $this->ga_loc =2;
			...
			if ( preg_match_all( '#<\s*link\s+rel\s*=\s*"stylesheet".+>#smUi',$html,$_css_links )>0 ) {
		...
		Protected function write_css( $files = array(),$the_rest=''){
			...
			$dir = $_SERVER['DOCUMENT_ROOT'] . $this->dir_css;
			// Get the files
		...
		Protected function write_js( $files = array(),$the_rest=''){
			...
			$dir = $_SERVER['DOCUMENT_ROOT'] . $this->dir_js;
			// Get the files
