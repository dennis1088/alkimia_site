...
control.current=this;this.data('rating',control);this.rating('draw');if(control.callback)control.callback.apply(control.element,control.current?[control.current.text(),$('a',control.current)[0]]:['']);},readOnly:function(toggle,disable){var control=this.data('rating');if(!control)return this;control.readOnly=toggle?true:false;if(disable)control.element.attr("disabled","disabled");else control.element.removeAttr("disabled");this.data('rating',control);this.rating('draw');},disable:function(){this.rating('readOnly',true,true);},enable:function(){this.rating('readOnly',false,false);}});$.fn.rating.options={cancel:'Cancel Rating',cancelValue:'',split:0,starWidth:16};$(function(){$('input[type=radio].star').rating();});})(jQuery);;;$(function(){ccm_getDashboardBackgroundImageData('20120623.jpg',false);});$(function(){$("#cDatePublic_dt").datepicker({dateFormat:'m/d/yy',changeYear:true,showAnim:'fadeIn'});});ccm_addHeaderItem("/concrete5/concrete/blocks/image/auto.js",'JAVASCRIPT');$(function(){$('#ccm-block-form').each(function(){ccm_setupBlockForm($(this),false,'add');});});$(function(){ccm_triggerSelectFile(2,'ccm-b-image-171');});ccm_triggerSelectFileComplete=function(fID,af){var td=$("#ccm-image-composer-thumbnail-"+af.substring(12));ccm_alGetFileData(fID,function(data){crop=false;td.html('');dw=data[0].width;dh=data[0].height;var tw=$("#ccm-image-composer-thumbnail-"+af.substring(12)).attr("target-width");var th=$("#ccm-image-composer-thumbnail-"+af.substring(12)).attr("target-height");if(tw!=dw){dw='<span style="color: #f00">'+dw+'</span>';crop=true;}
if(th!=dh){dh='<span style="color: #f00">'+dh+'</span>';crop=true;}
td.append('<strong>Actual Width</strong>: '+dw+'<br/>');td.append('<strong>Actual Height</strong>: '+dh+'<br/>');td.append('<a href="/concrete5/index.php/tools/blocks/image/crop_image?bID='+af.substring(12)+'&width='+tw+'&height='+th+'&fID='+fID+'" class="dialog-launch" dialog-modal="false" dialog-width="95%" dialog-height="460" dialog-title="Crop Image" class="dialog-launch" id="cropper-dialog-'+af.substring(12)+'">Crop + Upload Image</a>');$("#cropper-dialog-"+af.substring(12)).dialog();if(crop){$("#cropper-dialog-"+af.substring(12)).trigger('click');}});}
ThumbnailBuilder_onSaveCompleted=function(r){r=eval('('+r+')');console.log(r);jQuery.fn.dialog.closeTop();ccm_triggerSelectFile(r.fID,'ccm-b-image-'+r.bID,false);}
ccm_addHeaderItem("/concrete5/concrete/blocks/image/auto.js",'JAVASCRIPT');$(function(){$('#ccm-block-form').each(function(){ccm_setupBlockForm($(this),false,'add');});});$(function(){ccm_triggerSelectFile(8,'ccm-b-image-172');});ccm_triggerSelectFileComplete=function(fID,af){var td=$("#ccm-image-composer-thumbnail-"+af.substring(12));ccm_alGetFileData(fID,function(data){crop=false;td.html('');dw=data[0].width;dh=data[0].height;var tw=$("#ccm-image-composer-thumbnail-"+af.substring(12)).attr("target-width");var th=$("#ccm-image-composer-thumbnail-"+af.substring(12)).attr("target-height");if(tw!=dw){dw='<span style="color: #f00">'+dw+'</span>';crop=true;}
if(th!=dh){dh='<span style="color: #f00">'+dh+'</span>';crop=true;}
td.append('<strong>Actual Width</strong>: '+dw+'<br/>');td.append('<strong>Actual Height</strong>: '+dh+'<br/>');td.append('<a href="/concrete5/index.php/tools/blocks/image/crop_image?bID='+af.substring(12)+'&width='+tw+'&height='+th+'&fID='+fID+'" class="dialog-launch" dialog-modal="false" dialog-width="95%" dialog-height="460" dialog-title="Crop Image" class="dialog-launch" id="cropper-dialog-'+af.substring(12)+'">Crop + Upload Image</a>');$("#cropper-dialog-"+af.substring(12)).dialog();if(crop){$("#cropper-dialog-"+af.substring(12)).trigger('click');}});}
ThumbnailBuilder_onSaveCompleted=function(r){r=eval('('+r+')');console.log(r);jQuery.fn.dialog.closeTop();ccm_triggerSelectFile(r.fID,'ccm-b-image-'+r.bID,false);}
$(function(){$('#ccm-block-form').each(function(){ccm_setupBlockForm($(this),false,'add');});});$(function(){tinyMCE.init({mode:"textareas",width:"100%",height:"380px",inlinepopups_skin:"concreteMCE",entity_encoding:'raw',theme_concrete_buttons2_add:"spellchecker",relative_urls:false,document_base_url:'http://localhost/concrete5/',convert_urls:false,theme:"concrete",plugins:"paste,inlinepopups,spellchecker,safari,advlink,advimage,advhr",editor_selector:"ccm-advanced-editor-block_173",spellchecker_languages:"+English=en"});});$(function(){ccm_activateFileSelectors();});$(function(){var availableTags=null;$("#newAttrValueRows13").autocomplete({source:"/concrete5/index.php/tools/required/attribute_type_actions?atID=8&akID=13&action=load_autocomplete_values",select:function(event,ui){ccmAttributeTypeSelectTagHelper13.add(ui.item.value);$(this).val('');return false;}});$("#newAttrValueRows13").bind("keydown",function(e){if(e.keyCode==13){if($(this).val().length>0){ccmAttributeTypeSelectTagHelper13.add($(this).val());$(this).val('');$("#newAttrValueRows13").autocomplete("close");}
