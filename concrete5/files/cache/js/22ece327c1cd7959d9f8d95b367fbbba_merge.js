...
control.current=this;this.data('rating',control);this.rating('draw');if(control.callback)control.callback.apply(control.element,control.current?[control.current.text(),$('a',control.current)[0]]:['']);},readOnly:function(toggle,disable){var control=this.data('rating');if(!control)return this;control.readOnly=toggle?true:false;if(disable)control.element.attr("disabled","disabled");else control.element.removeAttr("disabled");this.data('rating',control);this.rating('draw');},disable:function(){this.rating('readOnly',true,true);},enable:function(){this.rating('readOnly',false,false);}});$.fn.rating.options={cancel:'Cancel Rating',cancelValue:'',split:0,starWidth:16};$(function(){$('input[type=radio].star').rating();});})(jQuery);;;$(function(){ccm_getDashboardBackgroundImageData('20120624.jpg',false);});var ccmActivePageField;function ccm_initSelectPage(){$("a.ccm-sitemap-select-page").unbind().dialog().click(function(){ccmActivePageField=this;});$("a.ccm-sitemap-clear-selected-page").unbind().click(function(){ccmActivePageField=this;clearPageSelection();});};function clearPageSelection(){var fieldName=$(ccmActivePageField).attr("dialog-sender");var par=$(ccmActivePageField).parent().find('.ccm-summary-selected-item-label');$(ccmActivePageField).parent().find('.ccm-sitemap-clear-selected-page').hide();var pari=$(ccmActivePageField).parent().find("[name='"+fieldName+"']");console.log(pari);par.html("");pari.val("0");}
$(ccm_initSelectPage);ccm_selectSitemapNode=function(cID,cName){var fieldName=$(ccmActivePageField).attr("dialog-sender");var par=$(ccmActivePageField).parent().find('.ccm-summary-selected-item-label');$(ccmActivePageField).parent().find('.ccm-sitemap-clear-selected-page').show();var pari=$(ccmActivePageField).parent().find("[name='"+fieldName+"']");par.html(cName);pari.val(cID);}
ccm_setupComposerFields=function(){if($("input[name=ctIncludeInComposer]").prop('checked')){$(".row-composer input, .row-composer select").attr('disabled',false);}else{$(".row-composer input, .row-composer select").attr('disabled','true');}
var val=$('input[name=ctComposerPublishPageMethod]:checked').val();switch(val){case'PAGE_TYPE':$("#ccm-composer-choose-parent-page-type").show();$("#ccm-composer-choose-parent").hide();break;case'PARENT':$("#ccm-composer-choose-parent-page-type").hide();$("#ccm-composer-choose-parent").show();break;default:$("#ccm-composer-choose-parent-page-type").hide();$("#ccm-composer-choose-parent").hide();break;}
$("div.ccm-composer-content-item-list").sortable({handle:'img.ccm-composer-content-item-icon',cursor:'move',opacity:0.5,stop:function(){var ualist=$(this).sortable('serialize');$.post('/concrete5/index.php/dashboard/pages/types/composer/save_content_items/4/',ualist,function(r){});}});}
$(function(){$("input[name=ctIncludeInComposer], input[name=ctComposerPublishPageMethod]").click(function(){ccm_setupComposerFields();});ccm_setupComposerFields();});